...
static void applyAspectRatio(_GLFWwindow* window, int edge, RECT* area)
{
    ...
    {
        area->bottom =
            area->top + (frame.bottom - frame.top) +
            (int)(((area->right - area->left) - (frame.right - frame.left)) /
                  ratio);
    }
    else if (edge == WMSZ_TOPLEFT || edge == WMSZ_TOPRIGHT)
    {
        area->top =
            area->bottom - (frame.bottom - frame.top) -
            (int)(((area->right - area->left) - (frame.right - frame.left)) /
                  ratio);
    }
    ...
    {
        area->right =
            area->left + (frame.right - frame.left) +
            (int)(((area->bottom - area->top) - (frame.bottom - frame.top)) *
                  ratio);
    }
}
...
static void updateCursorImage(_GLFWwindow* window)
{
    ...
    {
        // NOTE: Via Remote Desktop, setting the cursor to NULL does not hide
        // it. HACK: When running locally, it is set to NULL, but when connected
        // via Remote
...
static void enableCursor(_GLFWwindow* window)
{
    ...
    releaseCursor();
    _glfwSetCursorPosWin32(window, _glfw.win32.restoreCursorPosX,
                           _glfw.win32.restoreCursorPosY);
    updateCursorImage(window);
}
...
static void acquireMonitor(_GLFWwindow* window)
{
    if (!_glfw.win32.acquiredMonitorCount)
    {
        ...
        SystemParametersInfoW(SPI_GETMOUSETRAILS, 0,
                                   ...
                                   LPARAM lParam)
{
    ...
    _GLFWtitlebarHoveredButton titlebarHoveredButton =
        (_GLFWtitlebarHoveredButton)GetWindowLongPtrW(hWnd, GWLP_USERDATA);
    ...
    switch (uMsg)
    {
        ...
        case WM_CREATE:
        {
            ...
            if (hasThickFrame)
            {
                ...
                // Inform the application of the frame change to force redrawing
                // of the new client area
                // Note: The new client area extends into the non-client area
                SetWindowPos(hWnd, NULL, sizeRect.left, sizeRect.top,
        ...
        case WM_DESTROY:
        {
            // Send request to terminate the window
            PostQuitMessage(0);
            return 0;
        }
        ...
        case WM_SYSCHAR:
        {
            ...
            else
            {
                ...
                if (wParam >= 0xdc00 && wParam <= 0xdfff)
                {
                    ...
                    {
                        codepoint += (window->win32.highSurrogate - 0xd800)
                                     << 10;
                ...
                _glfwInputChar(window, codepoint, getKeyMods(),
                               uMsg != WM_SYSCHAR);
            }
        ...
        case WM_SYSKEYUP:
        {
            int key, scancode;
            const int action =
                (HIWORD(lParam) & KF_UP) ? GLFW_RELEASE : GLFW_PRESS;
            ...
            if (action == GLFW_RELEASE && wParam == VK_SHIFT)
            {
                ...
                _glfwInputKey(window, GLFW_KEY_LEFT_SHIFT, scancode, action,
                              mods);
                _glfwInputKey(window, GLFW_KEY_RIGHT_SHIFT, scancode, action,
                              mods);
            }
        ...
        case WM_MOUSEHWHEEL:
        {
            ...
            _glfwInputScroll(
                window, -((SHORT)HIWORD(wParam) / (double)WHEEL_DELTA), 0.0);
            return 0;
        }
        ...
        case WM_SIZE:
        {
            ...
            // HACK: Inform the application that of the frame change to
            // force redrawing with the new client area
            // TODO: Try to move this to line 1252 (the if statement)
            RECT sizeRect;
            GetWindowRect(hWnd, &sizeRect);
        ...
        case WM_NCCALCSIZE:
        {
            ...
            // Handling this message is necessary to extend the client area
            // (paintable area) to cover the entire window, including the
            // titlebar region Ref:
            // https://docs.microsoft.com/en-us/windows/win32/dwm/customframe#extending-the-client-frame
            // Note: When wParam is TRUE, the return value should be 0 (removing
            // the standard frame)
            UINT dpi = GetDpiForWindow(hWnd);

            int frameX = GetSystemMetricsForDpi(SM_CXFRAME, dpi);
            int frameY = GetSystemMetricsForDpi(SM_CYFRAME, dpi);
            // Note: SM_CXPADDEDBORDER not supported on Windows XP/2000
        ...
        case WM_NCHITTEST:
        {
            ...
            // Check if hover button is on maximize to support SnapLayout on
            // (Win11)
            if (titlebarHoveredButton == TitlebarHoveredButton_Maximize)
                return HTMAXBUTTON;

            // Titlebar hittest detection fix (Win10/11)
            // Note: Adjustments made to the non-client area (WM_NCCALCSIZE)
            // break top border hittest detection
            UINT dpi = GetDpiForWindow(hWnd);
            int frameY = GetSystemMetricsForDpi(SM_CYFRAME, dpi);
            int padding = GetSystemMetricsForDpi(SM_CXPADDEDBORDER, dpi);
            POINT cursorPos = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
            ScreenToClient(hWnd, &cursorPos);
            if (cursorPos.y > 0 && cursorPos.y < frameY + padding)
                return HTTOP;

            // Handle client-side hit test to determine titlebar bounds
            int titlebarHitTest = 0;
            _glfwInputTitlebarHitTest(window, cursorPos.x, cursorPos.y,
                                      &titlebarHitTest);
            if (titlebarHitTest)
                return HTCAPTION;
        ...
        case WM_MOVE:
        {
            ...
            _glfwInputWindowPos(window, GET_X_LPARAM(lParam),
                                GET_Y_LPARAM(lParam));
            return 0;
        }
        ...
        case WM_GETMINMAXINFO:
        {
                ...
                window->maxheight != GLFW_DONT_CARE)
            {
                mmi->ptMaxTrackSize.x =
                    window->maxwidth + frame.right - frame.left;
                mmi->ptMaxTrackSize.y =
                    window->maxheight + frame.bottom - frame.top;
            }
        ...
        case WM_PAINT:
        {
            // Notify shared code that window contents need to be updated
        ...
        case WM_GETDPISCALEDSIZE:
        {
            ...
            if (_glfwIsWindows10Version1703OrGreaterWin32())
            {
                ...
                size->cx +=
                    (target.right - target.left) - (source.right - source.left);
                size->cy +=
                    (target.bottom - target.top) - (source.bottom - source.top);
                return TRUE;
            }
        ...
        {
            const float xscale =
                HIWORD(wParam) / (float)USER_DEFAULT_SCREEN_DPI;
            const float yscale =
                LOWORD(wParam) / (float)USER_DEFAULT_SCREEN_DPI;
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    if (GetSystemMetrics(SM_REMOTESESSION))
    {
        ...
        //       When not on Remote Desktop, this handle is NULL and normal
        //       hiding is used
        if (!_glfw.win32.blankCursor)
        {
            ...
            unsigned char* cursorPixels =
                _glfw_calloc(cursorWidth * cursorHeight, 4);
            if (!cursorPixels)
                return GLFW_FALSE;

            // NOTE: Windows checks whether the image is fully transparent and
            // if so
            //       just ignores the alpha channel and makes the whole cursor
            //       opaque
            // HACK: Make one pixel slightly less transparent
            cursorPixels[3] = 1;

            const GLFWimage cursorImage = {cursorWidth, cursorHeight,
                                           cursorPixels};
    ...
    window->win32.handle = CreateWindowExW(
        exStyle, MAKEINTATOM(_glfw.win32.mainWindowClass), wideTitle, style,
    ...
    if (IsWindows7OrGreater())
    {
        ChangeWindowMessageFilterEx(window->win32.handle, WM_DROPFILES,
                                    MSGFLT_ALLOW, NULL);
        ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYDATA,
                                    MSGFLT_ALLOW, NULL);
    ...
    if (!window->monitor)
    {
        ...
        // This cannot be done until we know what monitor the window was placed
        // on Only update the restored window rect as the window may be
        // maximized
        ...
        // Adjust rect of maximized undecorated window, because by default
        // Windows will make such a window cover the whole monitor instead of
        // its workarea
    ...
    _glfwGetWindowSizeWin32(window, &window->win32.width,
                            &window->win32.height);
...
void _glfwSetWindowIconWin32(_GLFWwindow* window, int count,
                             const GLFWimage* images)
{
    ...
    SendMessageW(window->win32.handle, WM_SETICON, ICON_SMALL,
                 (LPARAM)smallIcon);
...
void _glfwSetWindowSizeLimitsWin32(_GLFWwindow* window, int minwidth,
                                   int minheight, int maxwidth, int maxheight)
{
    ...
    MoveWindow(window->win32.handle, area.left, area.top,
...
void _glfwGetWindowFrameSizeWin32(_GLFWwindow* window, int* left, int* top,
...
void _glfwGetWindowContentScaleWin32(_GLFWwindow* window, float* xscale,
                                     float* yscale)
{
    const HANDLE handle =
        MonitorFromWindow(window->win32.handle, MONITOR_DEFAULTTONEAREST);
...
void _glfwShowWindowWin32(_GLFWwindow* window)
{
    ...
    {
        // NOTE: GLFW windows currently do not seem to match the Windows 10
        // definition of
        //       a main window, so even SW_SHOWDEFAULT does nothing
        //       This definition is undocumented and can change (source: Raymond
        //       Chen)
    ...
    if (handle)
    {
        ...
        if (window)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                _glfwInputKey(window, key, scancode, GLFW_RELEASE,
                              getKeyMods());
            }
        }
    }
...
GLFWbool _glfwCreateCursorWin32(_GLFWcursor* cursor, const GLFWimage* image,
                                int xhot, int yhot)
{
...
GLFWbool _glfwCreateStandardCursorWin32(_GLFWcursor* cursor, int shape)
{
    ...
    switch (shape)
    {
        ...
        default:
            _glfwInputError(GLFW_PLATFORM_ERROR,
                            "Win32: Unknown standard cursor");
            return GLFW_FALSE;
    }

    cursor->win32.handle = LoadImageW(NULL, MAKEINTRESOURCEW(id), IMAGE_CURSOR,
                                      0, 0, LR_DEFAULTSIZE | LR_SHARED);
    if (!cursor->win32.handle)
    {
        _glfwInputErrorWin32(GLFW_PLATFORM_ERROR,
...
void _glfwSetClipboardStringWin32(const char* string)
{
    ...
    // NOTE: Retry clipboard opening a few times as some other application may
    // have it
    //       open and also the Windows Clipboard History reads it after each
    //       update
...
EGLenum _glfwGetEGLPlatformWin32(EGLint** attribs)
{
    if (_glfw.egl.ANGLE_platform_angle)
    {
        ...
        if (_glfw.egl.ANGLE_platform_angle_opengl)
        {
            ...
            else if (_glfw.hints.init.angleType ==
